...
const messages = {
  ...
  zh: {
    ...
    '종로구': '钟路区', '중구': '中区', '용산구': '龙山区', '성동구': '城东区', '麻浦区': '麻浦区', '江南区': '江南区', '永登浦区': '永登浦区', '铜雀区': '铜雀区'
  }
};
...
const filteredPlaces = computed(() => {
  if (selectedCategory.value === '여행경로') {
    return allLocations.value.filter(place => place.source === '여행경로');
  }

  if (!selectedDistrict.value) return [];
  return allLocations.value.filter(place => {
    if (place.source !== '관광명소') return false;
    const targetText = `${place.addr1 || ''} ${place.district || ''} ${place.title || ''}`;
    return targetText.includes(selectedDistrict.value);
  });
});
  ...
  <div class="app-container">
    ...
    <div v-if="currentTab === 'home'">
      <section class="hero">
        ...
        <div class="search-box" v-if="selectedCategory !== '여행경로'">
      ...
      <main class="main-content">
        ...
        <div class="map-section">
          <div class="map-wrapper">
            ...
            <div v-if="!selectedDistrict && selectedCategory !== '여행경로'" class="map-overlay-tips">
              <p>🗺️ 구를 선택하시면 상큼한 지도가 나타나요!</p>
            </div>
          </div>
          
          <div v-if="selectedDistrict && currentTmi && selectedCategory !== '여행경로'" class="tmi-box">
        ...
        <div class="list-section">
          ...
          <h2 class="section-title">
            <span>{{ selectedCategory === '여행경로' ? t('여행경로') : (selectedDistrict ? t(selectedDistrict) : t('allSeoul')) }}</span> 
